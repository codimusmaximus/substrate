	...
	for rows.Next() {
		...
		query := fmt.Sprintf(`
			SELECT
				...
				EXTRACT(EPOCH FROM (NOW() - MIN(CASE WHEN t.state IN ('pending', 'sleeping') THEN r.created_at END)))::bigint as oldest_age,
				EXTRACT(EPOCH FROM (NOW() - MAX(CASE WHEN t.state IN ('pending', 'sleeping') THEN r.created_at END)))::bigint as newest_age
			FROM absurd.%s t
